...
try {
    ...
    foreach ($files as $file) {
        ...
        $index = 1;
        $query = "INSERT INTO $table (".implode(",", $column_names).") VALUES ";
        $values = "";
        ...
        while ($node = $streamer->getNode()) {
            ...
            if ($action == 'Manage') {
                $currentRow = insertData($simpleXmlNode, $column_names, $table);
                $flag = 0;
                $values .= '('.implode(",", $currentRow).'), ';
                if($index % MAX_QUERY_SIZE == 0) {
                  $flag = 1;
...
{
    echo $log['details'] .= showError($e->getMessage(), $currentRow);
    $log['status'] = 'ERROR';     
}
catch (Exception $e) {
...
function insertData($simpleXmlNode, $column_names, $table)
{
    $data1 = array();
    $data2 = array();
